...
import { ISpendsOptions, ISpends, ISavings } from 'src/app/interface/finance/finance.interface';
...
export class AddSpendsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.userService.getSpends().subscribe( (data:Array<ISpendsOptions>): void => {
  ...
  updateUserFinance(): void{
    this.userService.updateUserFinance().subscribe(data => {
      this.toastrService.success('Spend add ang user finance update', 'Success')
      this.closePopUp()
    }, err=> this.closeWithError(err))
  }
